...
dashboard/                  # React SPA (Cloudflare Pages / local extension server)
  src/
    pages/                  # Dashboard, Projects, Endpoints, Suggestions, Graph, AiChat
    layout/                 # Layout
    lib/
      api.ts                # REST client (VITE_API_URL → production, or same-origin for local)
...
extension/                  # VSCode extension
  src/                      # Extension backend (TypeScript)
    local-server.ts         # Lightweight HTTP server for embedded dashboard
...
- **"Open Dashboard"** button launches a full dashboard in the browser backed by your scan data — no remote API required
